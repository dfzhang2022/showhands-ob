...
RC Db::init(const char *name, const char *dbpath) {
  ...
  if (OB_FAIL(rc)) {
    LOG_WARN("failed to init clog manager. dbpath=%s, rc=%s", dbpath,
             strrc(rc));
    return rc;
  }
...
RC Db::create_table(const char *table_name, int attribute_count,
  ...
  rc = table->create(next_table_id_++, table_file_path.c_str(), table_name,
...
// drop table构建的逻辑 应该将文件进行删除 同时将db中的条目进行删除
RC Db::drop_table(const char *table_name) {
  ...
  if (iter != opened_tables_.end()) {
    ...
    rc = table->close();
    if (rc != RC::SUCCESS) {
      LOG_ERROR("Failed to close table %s.", table_name);
      return rc;
    }
    // delete table;
  }
  ...
  if (std::remove(table_meta_file_path.c_str()) == 0) {
    LOG_INFO("Delete file: %s succeed.", table_meta_file_path.c_str());
  } else {
    LOG_ERROR("Failed to delete table meta file: %s.",
              table_meta_file_path.c_str());
  }
  std::string table_data_file_path = table_data_file(path_.c_str(), table_name);
  // 使用 std::remove 函数删除文件
  if (std::remove(table_data_file_path.c_str()) == 0) {
    LOG_INFO("Delete file: %s succeed.", table_data_file_path.c_str());
  } else {
    LOG_ERROR("Failed to delete table data file: %s.",
              table_data_file_path.c_str());
  }
...
RC Db::open_all_tables() {
  ...
  for (const std::string &filename : table_meta_files) {
    LOG_INFO("Getting into open %s.", filename.c_str());
    Table *table = new Table();

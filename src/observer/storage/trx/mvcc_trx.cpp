...
RC MvccTrxKit::init() {
  fields_ = vector<FieldMeta>{
      FieldMeta("__trx_xid_begin", AttrType::INTS, 0 /*attr_offset*/,
                4 /*attr_len*/, 0, false /*visible*/),
      FieldMeta("__trx_xid_end", AttrType::INTS, 0 /*attr_offset*/,
                4 /*attr_len*/, 1, false /*visible*/)};
...
RC MvccTrx::insert_record(Table *table, Record &record) {
  ...
  rc = log_manager_->append_log(CLogType::INSERT, trx_id_, table->table_id(),
                                record.rid(), record.len(), 0 /*offset*/,
                                record.data());
  ASSERT(rc == RC::SUCCESS,
         "failed to append insert record log. trx id=%d, table id=%d, rid=%s, "
         "record len=%d, rc=%s",
         trx_id_, table->table_id(), record.rid().to_string().c_str(),
         record.len(), strrc(rc));
...
RC MvccTrx::delete_record(Table *table, Record &record) {
  ...
  ASSERT(end_xid > 0,
         "concurrency conflit: other transaction is updating this record. "
         "end_xid=%d, current trx id=%d, rid=%s",
  ...
  RC rc = log_manager_->append_log(CLogType::DELETE, trx_id_, table->table_id(),
                                   record.rid(), 0, 0, nullptr);
  ASSERT(rc == RC::SUCCESS,
         "failed to append delete record log. trx id=%d, table id=%d, rid=%s, "
         "record len=%d, rc=%s",
         trx_id_, table->table_id(), record.rid().to_string().c_str(),
         record.len(), strrc(rc));
...
RC MvccTrx::visit_record(Table *table, Record &record, bool readonly) {
  ...
  if (begin_xid > 0 && end_xid > 0) {
    ...
    if (readonly) {
      ...
      rc = (-end_xid != trx_id_) ? RC::LOCKED_CONCURRENCY_CONFLICT
                                 : RC::RECORD_INVISIBLE;
    }
  }
  return rc;
...
RC MvccTrx::commit_with_trx_id(int32_t commit_xid) {
  // TODO
  // 这里存在一个很大的问题，不能让其他事务一次性看到当前事务更新到的数据或同时看不到
  RC rc = RC::SUCCESS;
  started_ = false;
  ...
  LOG_TRACE("append trx commit log. trx id=%d, commit_xid=%d, rc=%s", trx_id_,
            commit_xid, strrc(rc));
  return rc;
}
  ...
  for (const Operation &operation : operations_) {
    switch (operation.type()) {
      case Operation::Type::INSERT: {
        ...
        Table *table = operation.table();
        // TODO
        // 这里虽然调用get_record好像多次一举，而且看起来放在table的实现中更好，
...
RC MvccTrx::redo(Db *db, const CLogRecord &log_record) {
  ...
  switch (log_record.log_type()) {
    case CLogType::INSERT: {
      const CLogRecordData &data_record = log_record.data_record();
      Record record;
      record.set_data(const_cast<char *>(data_record.data_),
                      data_record.data_len_);
    ...
    case CLogType::DELETE: {
      ...
      RC rc = table->visit_record(data_record.rid_, false /*readonly*/,
                                  record_updater);
      ASSERT(rc == RC::SUCCESS,
             "failed to get record while committing. rid=%s, rc=%s",
      ...
      operations_.insert(
          Operation(Operation::Type::DELETE, table, data_record.rid_));
    } break;
    ...
    default: {
      ASSERT(false, "unsupported redo log. log_record=%s",

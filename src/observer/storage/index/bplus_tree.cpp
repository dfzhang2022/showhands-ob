...
RC LeafIndexNodeHandler::move_first_to_end(LeafIndexNodeHandler &other,
  ...
  if (size() >= 1) {
    memmove(__item_at(0), __item_at(1),
            (static_cast<size_t>(size()) - 1) * item_size());
  }
  increase_size(-1);
  return RC::SUCCESS;
}
...
void LeafIndexNodeHandler::preappend(const char *item) {
  if (size() > 0) {
    memmove(__item_at(1), __item_at(0),
            static_cast<size_t>(size()) * item_size());
  }
...
RC InternalIndexNodeHandler::copy_from(const char *items, int num,
                                       DiskBufferPool *disk_buffer_pool) {
  memcpy(__item_at(this->size()), items,
         static_cast<size_t>(num) * item_size());
...
RC BplusTreeHandler::sync() {
  if (header_dirty_) {
    ...
    if (OB_SUCC(rc) && frame != nullptr) {
      ...
      LOG_WARN("failed to sync index header file. file_desc=%d, rc=%s",
               disk_buffer_pool_->file_desc(), strrc(rc));
      // TODO: ingore?
    }
  }
...
RC BplusTreeHandler::create(const char *file_name, const std::vector<const FieldMeta*> &fields_meta,
                            IndexType type, int internal_max_size /* = -1*/, int leaf_max_size /* = -1 */) {
  ...
  // 这里用了一个page存放index的文件信息
  IndexFileHeader *file_header = (IndexFileHeader *)pdata;
  for(int32_t i = 0 ; i < fields_meta.size() ; i++) {
    file_header->attrs_length.emplace_back(fields_meta[i]->len());
    file_header->attrs_type.emplace_back(fields_meta[i]->type());
    file_header->attrs_index.emplace_back(fields_meta[i]->index());
    attr_length += fields_meta[i]->len();
  }
  ...
  key_comparator_.init(file_header_.attrs_type, file_header_.attrs_length, 
                      file_header_.attrs_index, file_header_.attr_length);
  key_printer_.init(file_header_.attrs_type, file_header_.attrs_length, file_header_.attr_length);
...
bool BplusTreeHandler::validate_node_recursive(LatchMemo &latch_memo,
                                               Frame *frame) {
  bool result = true;
  IndexNodeHandler node(file_header_, frame);
  if (node.is_leaf()) {
...
bool BplusTreeHandler::validate_leaf_link(LatchMemo &latch_memo) {
  ...
  memcpy(prev_key.get(), leaf_node.key_at(leaf_node.size() - 1),
  ...
  while (result && next_page_num != BP_INVALID_PAGE_NUM) {
    rc = latch_memo.get_page(next_page_num, frame);
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to fetch next page. page num=%d, rc=%s", next_page_num,
               strrc(rc));
      return false;
    }
...
bool BplusTreeHandler::validate_tree() {
  ...
  LatchMemo latch_memo(disk_buffer_pool_);
  Frame *frame = nullptr;
  RC rc = latch_memo.get_page(file_header_.root_page,
                              frame);  // 这里仅仅调试使用，不加root锁
  if (rc != RC::SUCCESS) {
  ...
  if (!validate_node_recursive(latch_memo, frame) ||
      !validate_leaf_link(latch_memo)) {
...
RC BplusTreeHandler::find_leaf(LatchMemo &latch_memo, BplusTreeOperationType op,
                               const char *key, Frame *&frame) {
  auto child_page_getter = [this,
                            key](InternalIndexNodeHandler &internal_node) {
...
RC BplusTreeHandler::left_most_page(LatchMemo &latch_memo, Frame *&frame) {
  ...
  return find_leaf_internal(latch_memo, BplusTreeOperationType::READ,
                            child_page_getter, frame);
}
...
RC BplusTreeHandler::find_leaf_internal(
  ...
  RC rc = crabing_protocal_fetch_page(latch_memo, op, file_header_.root_page,
                                      true /* is_root_node */, frame);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to fetch root page. page id=%d, rc=%d:%s",
             file_header_.root_page, rc, strrc(rc));
    return rc;
  ...
  for (; !node->is_leaf;) {
    ...
    rc = crabing_protocal_fetch_page(latch_memo, op, next_page_id,
                                     false /* is_root_node */, frame);
    if (rc != RC::SUCCESS) {
      LOG_WARN("Failed to load page page_num:%d. rc=%s", next_page_id,
               strrc(rc));
      return rc;
    }
...
RC BplusTreeHandler::insert_entry_into_leaf_node(LatchMemo &latch_memo,
  ...
  return insert_entry_into_parent(latch_memo, frame, new_frame,
                                  new_index_node.key_at(0));
}

RC BplusTreeHandler::insert_entry_into_parent(LatchMemo &latch_memo,
  ...
  if (parent_page_num == BP_INVALID_PAGE_NUM) {
    ...
    if (parent_node.size() < parent_node.max_size()) {
      ...
      rc = split<InternalIndexNodeHandler>(latch_memo, parent_frame,
                                           new_parent_frame);
      if (rc != RC::SUCCESS) {
        ...
        rc = insert_entry_into_parent(latch_memo, parent_frame,
                                      new_parent_frame, new_node.key_at(0));
      }
    }
  }
  return rc;
...
RC BplusTreeHandler::split(LatchMemo &latch_memo, Frame *frame,
                           Frame *&new_frame) {
  ...
  old_node.move_half_to(new_node,
                        disk_buffer_pool_);  // TODO remove disk buffer pool
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const char *user_key,
                                                   const RID &rid) {
  ...
  memcpy(static_cast<char *>(key.get()) + file_header_.attr_length, &rid,
         sizeof(rid));
  return key;
}
...
RC BplusTreeHandler::insert_entry(const char *user_key, const RID *rid) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_TRACE("Failed to insert into leaf of index, rid:%s. rc=%s",
...
RC BplusTreeHandler::get_entry(const char *user_key, int key_len,
  ...
  scanner.close();
  // Get = RC::SUCCESS, Not Get = RC::RECORD_EOF;
  if (rc != RC::RECORD_EOF) {
...
template <typename IndexNodeHandlerType>
RC BplusTreeHandler::coalesce_or_redistribute(LatchMemo &latch_memo,
                                              Frame *frame) {
  ...
  if (index_node.size() + neighbor_node.size() > index_node.max_size()) {
    ...
    rc = coalesce<IndexNodeHandlerType>(latch_memo, neighbor_frame, frame,
                                        parent_frame, index);
  }
...
template <typename IndexNodeHandlerType>
RC BplusTreeHandler::coalesce(LatchMemo &latch_memo, Frame *neighbor_frame,
  ...
  return coalesce_or_redistribute<InternalIndexNodeHandler>(latch_memo,
                                                            parent_frame);
}
...
RC BplusTreeHandler::delete_entry_internal(LatchMemo &latch_memo,
...
/**
 * 这里的user key需要包含null bit map。
 * 支持多列索引之后，这里的left_len和right_len指的是index所有字段的总长度。
 * 在fix_user_key时，使用Value(单个字段).length()，来实际获取截止到'\0'的长度。
 * 但实际上，char类型虽然实际长度可能不是字段设置的长度，但是生成record的时候，
 * 每个字段都是按字段记录的offset固定值存储。
 * 所以这里的left_len本意上应该是指实际长度，但实际上就是所有字段固定长度之和。
 * 这里就按理论的长度理解，暂时先不用，可能会涉及到变长字段的实现。
*/
RC BplusTreeScanner::open(const char *left_user_key, int left_len,
  ...
  if (nullptr == left_user_key) {
    ...
    int offset = 0;
    int key_len = tree_handler_.file_header_.attr_length;
    char *fixed_left_key = const_cast<char *>(left_user_key);
    for (int i = 0 ; i < tree_handler_.file_header_.attrs_type.size() ; i ++) {
      if (tree_handler_.file_header_.attrs_type[i] == CHARS) {
        bool should_inclusive_after_fix = false;
        char *fixed_left_key_cell = const_cast<char *>(left_user_key + offset);
        int cell_key_len = Value(fixed_left_key_cell).length();
        rc = fix_user_key(left_user_key + offset, cell_key_len, tree_handler_.file_header_.attrs_length[i], 
                          true /*greater*/,
                          &fixed_left_key_cell, &should_inclusive_after_fix);
        if (rc != RC::SUCCESS) {
        ...
        if (fixed_left_key_cell != left_user_key + offset) {
          char *tmp = (char*)malloc(key_len
                                     + (tree_handler_.file_header_.attrs_length[i] - cell_key_len));
          memcpy(tmp, left_user_key, offset);
          memcpy(tmp + offset, fixed_left_key_cell, tree_handler_.file_header_.attrs_length[i]);
          memcpy(tmp + offset + tree_handler_.file_header_.attrs_length[i], 
                  left_user_key, key_len - offset - cell_key_len);
          key_len +=  tree_handler_.file_header_.attrs_length[i] - cell_key_len;

          if(fixed_left_key != left_user_key) {
            free(fixed_left_key);
          }
          fixed_left_key = tmp;
          free(fixed_left_key_cell);
          fixed_left_key_cell = nullptr;
        }
      }

      offset += tree_handler_.file_header_.attrs_length[i];
    }
    ...
    rc = tree_handler_.find_leaf(latch_memo_, BplusTreeOperationType::READ,
                                 left_key, current_frame_);
    if (rc == RC::EMPTY) {
      rc = RC::SUCCESS;
  ...
  if (nullptr == right_user_key) {
    ...
    for (int i = 0 ; i < tree_handler_.file_header_.attrs_type.size() ; i ++) {
      if (tree_handler_.file_header_.attrs_type[i] == CHARS) {
        bool should_inclusive_after_fix = false;
        char *fixed_right_key_cell = const_cast<char *>(right_user_key + offset);
        int cell_key_len = Value(fixed_right_key_cell).length();
        rc = fix_user_key(right_user_key + offset, cell_key_len, tree_handler_.file_header_.attrs_length[i], 
                          true /*greater*/,
                          &fixed_right_key_cell, &should_inclusive_after_fix);
        if (rc != RC::SUCCESS) {
          LOG_WARN("failed to fix left user key. rc=%s", strrc(rc));
          return rc;
        }
        ...
        if (fixed_right_key_cell != right_user_key + offset) {
          char *tmp = (char*)malloc(key_len
                                     + (tree_handler_.file_header_.attrs_length[i] - cell_key_len));
          memcpy(tmp, right_user_key, offset);
          memcpy(tmp + offset, fixed_right_key_cell, tree_handler_.file_header_.attrs_length[i]);
          memcpy(tmp + offset + tree_handler_.file_header_.attrs_length[i], 
                  right_user_key, key_len - offset - cell_key_len);
          key_len +=  tree_handler_.file_header_.attrs_length[i] - cell_key_len;

          if(fixed_right_key != right_user_key) {
            free(fixed_right_key);
          }
          fixed_right_key = tmp;
          free(fixed_right_key_cell);
          fixed_right_key_cell = nullptr;
        }
      }
...
RC BplusTreeScanner::next_entry(RID &rid) {
  ...
  rc = latch_memo_.get_page(next_page_num, current_frame_);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to get next page. page num=%d, rc=%s", next_page_num,
             strrc(rc));
    return rc;
  }
...
RC BplusTreeScanner::fix_user_key(const char *user_key, int key_len, int attr_len,

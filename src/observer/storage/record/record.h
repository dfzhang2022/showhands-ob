...
struct RID {
   ...
   * 0通常用于存放meta数据，所以对数据部分来说都是 不合法的. 这里在bplus
   * tree中查找时会用到。
   */
  static RID *min() {
    static RID rid{0, 0};
...
class Record {
 public:
  ...
  Record *clone() {
    Record *ret_record = new Record();
    ret_record->set_rid(this->rid());
    char *tmp_data = (char *)malloc(this->len());
    memcpy(tmp_data, this->data(), this->len());
    ret_record->set_data(tmp_data, this->len());
    return ret_record;
  }

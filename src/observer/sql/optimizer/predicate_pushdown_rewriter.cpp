 ...
 * 判断一个子树是否包含该表达式取值的表
 */
RC PredicatePushdownRewriter::contain_expr(LogicalOperator *oper, std::unique_ptr<Expression> &expr, bool &contain_result)
{
  RC rc = RC::SUCCESS;
  ...
  if (oper->type() == LogicalOperatorType::TABLE_GET) {
    ...
    std::vector<std::unique_ptr<LogicalOperator>> &children_opers = join_oper->children();
    for (auto iter = children_opers.begin(); iter != children_opers.end(); iter++) {
      ...
      if (left_oper_result) {
        if (right_oper_result) {
          ...
          if (left_oper->type() == LogicalOperatorType::TABLE_GET) {
            ...
            if (!pushdown_exprs.empty()) {
              auto table_get_oper = static_cast<TableGetLogicalOperator *>(left_oper.get());
              table_get_oper->set_predicates(std::move(pushdown_exprs));
            }
            else {
            ...
            auto child_join_oper = static_cast<JoinLogicalOperator *>(left_oper.get());
        ...
        if (right_oper_result) {
          ...
          if (right_oper->type() == LogicalOperatorType::TABLE_GET) {
            ...
            if (!pushdown_exprs.empty()) {
              auto table_get_oper = static_cast<TableGetLogicalOperator *>(right_oper.get());
              table_get_oper->set_predicates(std::move(pushdown_exprs));
            }
            else {
              pushdown_result = false;
            ...
            auto child_join_oper = static_cast<JoinLogicalOperator *>(right_oper.get());
            rc = join_exprs_can_pushdown(child_join_oper, expr, pushdown_result);
            if (rc != RC::SUCCESS) {
              return rc;
            }

...
%type <sql_node>            join_list
%type <sql_node>            rel_list
...
select_stmt:        /*  select 语句的语法解析树*/
    SELECT select_attr FROM ID join_list rel_list where
    {
      ...
      if ($5 != nullptr) {
        $$->selection.relations.insert($$->selection.relations.end(),
          $5->selection.relations.begin(), $5->selection.relations.end());
      }
      ...
      if ($5 != nullptr) {
        $$->selection.conditions.insert($$->selection.conditions.end(),
          $5->selection.conditions.begin(), $5->selection.conditions.end());
      }
    }
    ;
calc_stmt:
...
rel_list:
    ...
    | COMMA ID join_list rel_list {
      if ($4 != nullptr) {
        ...
        if ($3 != nullptr) {
          $$->selection.relations.insert($$->selection.relations.end(),
            $3->selection.relations.begin(), $3->selection.relations.end());
          $$->selection.conditions.insert($$->selection.conditions.end(),
            $3->selection.conditions.begin(), $3->selection.conditions.end());
        }
      } else {
        if ($3 != nullptr) {
          $$ = $3;

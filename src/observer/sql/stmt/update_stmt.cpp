  ...
  for (int i = 0; i < value_amount; i++) {
    ...
    if(update_sql.update_values.at(i).is_right_selects){
      // right value is select statement
        Stmt *stmt = nullptr;
        rc = SelectStmt::create(db, update_sql.update_values[i].select_sql_node, stmt);
        if (rc != RC::SUCCESS && rc != RC::UNIMPLENMENT) {
          LOG_WARN("failed to create select_stmt in update_stmt. rc=%d:%s", rc, strrc(rc));
          return rc;
        }
        SelectStmt* select_stmt_ptr = static_cast<SelectStmt*> (stmt);
        
        col_name_to_selects.insert(std::pair<std::string,SelectStmt*>(attr_name,select_stmt_ptr));
        val_vec.emplace_back(Value(0));
    }
    else val_vec.emplace_back(update_sql.update_values.at(i).value);
  }

  // LOG_INFO("value is %d", update_sql.value.get_int());
  // stmt = new UpdateStmt(table, update_sql.attribute_name, &update_sql.value,
  //                       int(1), filter_stmt);
  stmt = new UpdateStmt(table, str_vec, val_vec, value_amount, filter_stmt,col_name_to_selects);
  return rc;
}
